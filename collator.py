...
def main():
	...
	parser.add_argument("-eat", "--exit_after_table_creation", default=False,
						help="Exit after alignment table production.")
	...
	parser.add_argument("-sp", "--structuring_proportion", default=0.3,
						help="Ratio to cut the text for automatic pre-alignment (default: .3)")
	...
	if structuration_automatique:
		...
		# all_wits = glob.glob('temoins_tokenises_regularises*/*.xml') + glob.glob('temoins_tokenises/*.xml')
		# for file in all_wits:
	...
	for localisation in portee:
		...
		for file in glob.glob(f"divs/div{localisation}/*transposed.xml"):
			...
			print(sigle)
			utils.clean_xml_file(input_file=file,
								 output_file=f"divs/div{localisation}/apparat_{sigle}_{localisation}_final.xml")
