...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:tei="http://www.tei-c.org/ns/1.0">
    ...
    <sch:pattern>
        ...
        <sch:rule
            context="tei:note[not(ancestor::tei:handShift)][ancestor::tei:body][ancestor::tei:TEI[@type = 'transcription']]">
    ...
    <sch:pattern>
        ...
        <sch:rule context="tei:w">
            ...
            <sch:assert test="not(descendant::tei:w)" role="fatal">Interdit d'enchâsser deux tei:w
                !</sch:assert>
            <sch:assert role="fatal" test="not(ancestor::tei:add[@type = 'commentaire'])">Pas de tei:w
                dans une transcription de glose ou de commentaire!</sch:assert>
        </sch:rule>
    </sch:pattern>
</sch:schema>
